...
export function SectionTracker({ children, userId, articleId }: SectionTrackingProps) {
  ...
  useEffect(() => {
    ...
    console.log('SectionTracker: Article element found?', !!articleElement);
    
    if (articleElement) {
      // Observe all existing headings
      const existingHeadings = articleElement.querySelectorAll('h1, h2, h3, h4, h5, h6');
      console.log('SectionTracker: Found headings', existingHeadings.length);
